...
public class MixpanelFlutterPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = MixpanelFlutterPlugin()
    ...
    private func getMixpanelInstance(_ token: String) -> MixpanelInstance? {
        ...
        if instance == nil {
            let options = MixpanelOptions(
                ...
                flushInterval: defaultFlushInterval,
                instanceName: token,
                trackAutomaticEvents: trackAutomaticEvents ?? false
            )
            instance = Mixpanel.initialize(options: options)
        }
    ...
    private func handleUpdateFlagsContext(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        NSLog("[Mixpanel] updateFlagsContext is not supported on macOS. Feature flags context must be set during initialization.")
        result(nil)
    }
